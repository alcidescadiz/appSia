<div>
    <h3>Detalles de factura a editar:</h3>
    ...
    <table class="table table-striped table-inverse table-responsive"  style="margin:5px; padding: 5px; width: 95% ">
        ...
        <tbody class="thead-inverse">
            ...
            <tr>
                <td data-titulo="Prducto:"> {{$item->nombre}} </td>
                <td data-titulo="Costo:"> {{$item->costo}}  </td>
                <td data-titulo="Cantidad:">{{$item->cantidad}}  </td>
                <td data-titulo="Iva:"> {{$item->iva}} </td>
                <td data-titulo="Subtatel:">{{$item->subtotal}} </td>
                <td>
                    <form method="POST" action="{{ url("detallesedit/{$item->id}") }}">
                        @csrf
                        @method('DELETE')
            ...
            @empty
                <tr>
                    <th colspan="6">No hay detalles en la compra</th>
               </tr> 
            @endforelse
        </tbody>
    </table>
